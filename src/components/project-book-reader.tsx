...
import { renderFuriganaExample, renderFuriganaText } from "@/components/furigana";
...
export function ProjectBookReader({ sections, units, config }: Props) {
  ...
  const nextChapterIndex = sections.findIndex((_,index) => index > 0 && !completed.includes(index));
  ...
  return <div className={styles.project}>
    ...
    <section className={`${styles.reader} ${config.unitViewsInModal ? styles.tocOnly : ""}`} id="digital-book">
      <aside className={styles.contents}>
        ...
        <div className={styles.contentsProgress}><i style={{width:`${progress}%`}}/></div>
        <p className={styles.contentsIntro}>একটি Unit-এর ছয়টি অংশ ধারাবাহিকভাবে শেষ করুন—পড়া হলে check mark যোগ হবে এবং progress এই browser-এ সংরক্ষিত থাকবে।</p>
        <blockquote className={styles.contentsQuote}><b>毎日、少しずつ。</b><span>প্রতিদিন অল্প অল্প করে—কিন্তু ধারাবাহিকভাবে।</span></blockquote>
        ...
        <nav aria-label="Book chapters">{sections.map((section,index) => <div className={singleStyles.unitGroup} key={section.title}><button className={activeChapter === index ? styles.current : ""} onClick={() => openChapter(index, "lesson", Boolean(config.unitViewsInModal && index === 0))}><b>{index === 0 ? "শুরু" : String(unitNumberFor(index)).padStart(2,"0")}</b><span><strong>{index === 0 ? "পড়ার আগে" : `Unit ${String(unitNumberFor(index)).padStart(2,"0")}`}</strong><small>{section.title.replace(/^Unit\s+\d+\s*[·-]\s*/i, "")}</small></span><i>{completed.includes(index) ? "✓" : index === nextChapterIndex ? "→" : ""}</i></button>{activeChapter === index && index > 0 ? <div className={singleStyles.unitMenu}>{unitViews.map((view,viewIndex) => <button className={unitView === view.id ? singleStyles.activeUnitMenu : ""} onClick={() => openChapter(index, view.id, config.unitViewsInModal)} key={view.id} aria-haspopup={config.unitViewsInModal ? "dialog" : undefined} disabled={Boolean(config.unitViewsInModal && viewIndex > unlockedViewForChapter(index))}><b>{view.icon}</b><span>{view.label}</span></button>)}</div> : null}</div>)}</nav>
      </aside>
      ...
      <div ref={modalShellRef} className={`${styles.bookShell} ${isModalOpen ? styles.modalShell : ""}`} role={isModalOpen ? "dialog" : undefined} aria-modal={isModalOpen || undefined} aria-label={isModalOpen && chapter ? `${chapter.title} · ${unitViews.find((item) => item.id === unitView)?.label}` : undefined}>
        ...
        {chapterIsVisible && chapter && guide ? <>
          <div className={`${styles.spread} ${singleStyles.spread}`}>
            <article className={`${styles.page} ${singleStyles.page}`}>
              ...
              {!unit || unitView === "lesson" ? unitLessonDetails && unit ? <section className={singleStyles.lessonDashboard}>
                <header className={singleStyles.lessonHero}><div><small>{unitLessonDetails.label}</small><span>UNIT {String(unit.number).padStart(2,"0")} · LESSON</span><h2>{unitLessonDetails.headline}</h2><p>{unitLessonDetails.summary}</p></div><aside><b>এই Lesson-এর ফলাফল</b><p>{unitLessonDetails.outcome}</p></aside></header>
              ...
              </section> : config.bookId === "n5" ? <section className={singleStyles.foundationDashboard}>
                <header className={singleStyles.foundationHero}><div><small>{minnaN5FoundationDashboard.label}</small><span>START · 00</span><h2>{minnaN5FoundationDashboard.title}</h2><p>{minnaN5FoundationDashboard.summary}</p></div><aside><b>এই Foundation শেষে</b><p>{minnaN5FoundationDashboard.outcome}</p><em>প্রায় ২০–৩০ মিনিট</em></aside></header>
                <section className={singleStyles.foundationScripts}><header><small>01 · WRITING SYSTEM</small><h3>Japanese-এর তিনটি script</h3><p>কাজ আলাদা, কিন্তু একই sentence-এ পাশাপাশি থাকে।</p></header><div>{minnaN5FoundationDashboard.scripts.map((script) => <article className={singleStyles[`foundation_${script.tone}`]} key={script.name}><b>{script.mark}</b><span><small>{script.japanese}</small><strong>{script.name}</strong><i>{script.count}</i><p>{script.use}</p></span></article>)}</div></section>
                <section className={singleStyles.foundationSounds}><header><small>02 · SOUND MAP</small><h3>মাত্র পাঁচটি vowel sound</h3><p>প্রতিবার একইভাবে উচ্চারণ করুন—English-এর মতো sound বদলায় না।</p></header><div>{minnaN5FoundationDashboard.vowels.map((vowel) => <article key={vowel.kana}><b>{vowel.kana}</b><strong>{vowel.sound}</strong><small>{vowel.cue}</small></article>)}</div></section>
                <section className={singleStyles.foundationSentence}><header><small>03 · YOUR FIRST SENTENCE</small><h3>Slot বদলালেই নতুন বাক্য</h3></header><div className={singleStyles.foundationFormula}><span><b>わたし</b><small>আমি / Topic</small></span><em>は</em><span><b>ハリム</b><small>নিজের নাম</small></span><em>です</em></div><strong>わたしは ハリムです。</strong><i>Watashi wa Harimu desu.</i><p>আমি হালিম। নিজের নাম বসিয়ে sentence-টি তিনবার বলুন।</p></section>
                <section className={singleStyles.foundationParticles}><header><small>04 · ESSENTIAL MARKERS</small><h3>Particle—ছোট marker, বড় কাজ</h3></header><div>{minnaN5FoundationDashboard.particles.map((particle) => <article key={particle.japanese}><b>{particle.japanese}</b><span><strong>{particle.role}</strong><i>{particle.example}</i></span><small>{particle.reading}</small></article>)}</div></section>
                <section className={singleStyles.foundationRoutine}><header><small>05 · HOW TO STUDY</small><h3>প্রতি Unit-এ এই ক্রমে এগোন</h3></header><div>{minnaN5FoundationDashboard.routine.map((step,index) => <article key={step.title}><b>{step.icon}</b><span><em>{String(index + 1).padStart(2,"0")}</em><strong>{step.title}</strong><p>{step.note}</p></span></article>)}</div></section>
                <section className={singleStyles.foundationChecklist}><header><small>READY CHECK</small><h3>Unit 01-এ যাওয়ার আগে</h3><p>সবগুলো perfect না হলেও ধারণা পরিষ্কার হলেই এগিয়ে যান।</p></header><div>{minnaN5FoundationDashboard.checklist.map((item) => <span key={item}>✓ {item}</span>)}</div></section>
                <details className={singleStyles.lessonDeepRead}><summary><span>বিস্তারিত Foundation explanation ও study notes</span><b>খুলুন ＋</b></summary><div className={singleStyles.lessonDeepBody}><section className={singleStyles.lessonDeepContent}><header><span>01</span><div><small>DETAILED FOUNDATION</small><h3>Script, উচ্চারণ ও word order</h3></div></header><div className={`${styles.bookText} ${singleStyles.lessonDeepRich}`}><RichTextContent content={guide.overview || guide.content || ""}/></div></section><section className={`${singleStyles.lessonDeepContent} ${singleStyles.lessonDeepNotes}`}><header><span>02</span><div><small>MINI PRACTICE</small><h3>আজকের practice ও মনে রাখার বিষয়</h3></div></header><div className={`${styles.bookText} ${singleStyles.lessonDeepRich}`}><RichTextContent content={guide.studyNotes || practice?.overview || ""}/></div></section></div></details>
              ...
              {unit && unitView === "vocabulary" ? <section className={singleStyles.unitSection}>
                ...
                <div className={singleStyles.vocabGrid}>{vocabularyCards.map(({item,index,memoryKey}) => { const [word,...meaning] = item.split("—"); const cleanWord = word.trim(); const mnemonic = config.bookId === "n5" && unit.number === 1 ? { columns:8, rows:5, image:"/images/projects/n5-vocabulary/unit-01-sprites.png" } : config.bookId === "n5" && unit.number === 2 ? { columns:6, rows:5, image:"/images/projects/n5-vocabulary/unit-02-sprites.png" } : config.bookId === "n5" && unit.number === 3 ? { columns:6, rows:6, image:"/images/projects/n5-vocabulary/unit-03-sprites.png" } : config.bookId === "n5" && unit.number === 4 ? { columns:7, rows:8, image:"/images/projects/n5-vocabulary/unit-04-sprites.png" } : config.bookId === "n5" && unit.number === 5 ? { columns:10, rows:6, image:"/images/projects/n5-vocabulary/unit-05-sprites.png" } : config.bookId === "n5" && unit.number === 6 ? { columns:10, rows:6, image:"/images/projects/n5-vocabulary/unit-06-sprites.png" } : config.bookId === "n5" && unit.number === 7 ? { columns:10, rows:8, image:"/images/projects/n5-vocabulary/unit-07-sprites.png" } : config.bookId === "n5" && unit.number === 8 ? { columns:8, rows:8, image:"/images/projects/n5-vocabulary/unit-08-sprites.png" } : config.bookId === "n5" && unit.number === 9 ? { columns:8, rows:7, image:"/images/projects/n5-vocabulary/unit-09-sprites.png" } : config.bookId === "n5" && unit.number === 10 ? { columns:10, rows:6, image:"/images/projects/n5-vocabulary/unit-10-sprites.png" } : null; const example = config.bookId === "n5" ? getMinnaN5VocabularyExample(unit.number, index) : undefined; const spritePosition = mnemonic ? `${(index % mnemonic.columns) * (100 / (mnemonic.columns - 1))}% ${Math.floor(index / mnemonic.columns) * (100 / (mnemonic.rows - 1))}%` : undefined; const isRemembered = rememberedVocabulary.includes(memoryKey); const memoryButton = <button type="button" className={`${singleStyles.rememberButton} ${isRemembered ? singleStyles.rememberedButton : ""}`} onClick={() => toggleRememberedVocabulary(memoryKey)} aria-pressed={isRemembered} aria-label={`${cleanWord} ${isRemembered ? "মনে নেই হিসেবে রাখুন" : "মনে আছে হিসেবে রাখুন"}`} title={isRemembered ? "মনে আছে ✓" : "মনে আছে হিসেবে চিহ্নিত করুন"}>✓</button>; if (mnemonic && example) { const isFlipped = flippedVocabularyIndex === index; return <article className={`${singleStyles.vocabCardWrap} ${isRemembered ? singleStyles.rememberedCard : ""}`} key={item}>{memoryButton}<button type="button" className={`${singleStyles.vocabCard} ${singleStyles.vocabFlipCard} ${isFlipped ? singleStyles.flippedCard : ""}`} onClick={() => setFlippedVocabularyIndex(isFlipped ? null : index)} aria-pressed={isFlipped} aria-label={`${cleanWord} card—${isFlipped ? "শব্দ দেখুন" : "example sentence দেখুন"}`}><span className={singleStyles.vocabNumber}>{String(index + 1).padStart(2,"0")}</span><span className={singleStyles.vocabFlipInner}><span className={singleStyles.vocabCardFront}><span className={singleStyles.vocabImage} style={{backgroundImage:`url(${mnemonic.image})`,backgroundSize:`${mnemonic.columns * 100}% ${mnemonic.rows * 100}%`,backgroundPosition:spritePosition}} role="img" aria-label={`${cleanWord} মনে রাখার ছবি`}/><span className={singleStyles.vocabCopy}><strong>{renderFuriganaText(cleanWord)}</strong><small>{meaning.join("—").trim()}</small><i>Click করে example দেখুন ↻</i></span></span><span className={singleStyles.vocabCardBack}><small className={singleStyles.focusWordBadge}>Focus word · {renderFuriganaText(cleanWord)}</small><strong>{highlightedVocabulary(example.japanese, cleanWord)}</strong><span>{example.romaji}</span><p>{example.bengali}</p><i>শব্দে ফিরুন ↻</i></span></span></button></article>; } return <article className={`${singleStyles.vocabCardWrap} ${isRemembered ? singleStyles.rememberedCard : ""}`} key={item}>{memoryButton}<div className={singleStyles.vocabCard}><span className={singleStyles.vocabNumber}>{String(index + 1).padStart(2,"0")}</span><div className={singleStyles.vocabCopy}><h3>{renderFuriganaText(cleanWord)}</h3><p>{meaning.join("—").trim()}</p></div></div></article>; })}</div>
          ...
          <article className={`${styles.page} ${singleStyles.page} ${styles.indexPage}`}>
            <div className={styles.runningHead}><span>TABLE OF CONTENTS</span><b>{config.bookLabel} · {config.unitRangeLabel}</b></div>
            <header className={styles.indexHero}>
              <div><span className={styles.chapterLabel}>সূচিপত্র · STUDY MAP</span><h1>আপনার Japanese learning journey</h1><p className={styles.indexIntro}>একটি Unit-এর ছয়টি অংশ ধারাবাহিকভাবে শেষ করুন। পড়া অংশে check mark থাকবে এবং progress এই browser-এ সংরক্ষিত হবে।</p></div>
              <aside><div className={styles.progressRing} style={{"--course-progress":`${progress * 3.6}deg`} as CSSProperties}><strong>{progress}%</strong><small>COMPLETE</small></div><p><b>{completedUnitCount}</b> / {units.length} Units শেষ</p></aside>
            </header>
            <section className={styles.indexStats} aria-label="Course summary">
              <article><b>{units.length}</b><span><strong>Units</strong><small>N5 structured course</small></span></article>
              <article><b>6</b><span><strong>Parts per Unit</strong><small>Learn → listen → practice</small></span></article>
              <article><b>{nextChapterIndex > 0 ? String(unitNumberFor(nextChapterIndex)).padStart(2,"0") : "✓"}</b><span><strong>{nextChapterIndex > 0 ? "Next Unit" : "Course complete"}</strong><small>{nextChapterIndex > 0 ? "এখান থেকে চালিয়ে যান" : "সব Unit শেষ হয়েছে"}</small></span></article>
            </section>
            <section className={styles.indexLearningPath} aria-label="Learning sequence"><header><small>ONE CLEAR ROUTE</small><strong>প্রতি Unit-এর শেখার ধাপ</strong></header><div>{unitViews.map((view,index) => <span key={view.id}><b>{view.icon}</b><i>{view.label}</i>{index < unitViews.length - 1 ? <em>→</em> : null}</span>)}</div></section>
            <blockquote><b>毎日、少しずつ。</b><span>প্রতিদিন অল্প অল্প করে—কিন্তু ধারাবাহিকভাবে।</span></blockquote>
            <div className={styles.indexSectionHead}><div><small>COURSE MAP</small><strong>Chapter index</strong></div><span>{completedUnitCount}/{units.length} Units complete</span></div>
            <div className={styles.indexList}>{sections.map((section,index) => {
              const isCompleted = completed.includes(index);
              const isCurrent = activeChapter === index;
              return <div className={`${singleStyles.indexUnit} ${isCompleted ? styles.indexCompleted : ""} ${isCurrent ? styles.indexCurrent : ""}`} key={section.title}>
                <button className={isCurrent ? styles.current : ""} onClick={() => openChapter(index, "lesson", Boolean(config.unitViewsInModal && index === 0))}>
                  <b>{index === 0 ? "文" : String(unitNumberFor(index)).padStart(2,"0")}</b>
                  <span><em>{index === 0 ? "START HERE" : `UNIT ${String(unitNumberFor(index)).padStart(2,"0")}`}</em><strong>{index === 0 ? "Foundation · পড়ার আগে" : section.title.replace(/^Unit\s+\d+\s*[·-]\s*/i, "")}</strong><small>{chapterSummary[index]}</small></span>
                  <i>{isCompleted ? "✓" : "→"}</i>
                </button>
                {isCurrent && index > 0 ? <div className={singleStyles.unitMenu}>{unitViews.map((view,viewIndex) => <button className={unitView === view.id ? singleStyles.activeUnitMenu : ""} onClick={() => openChapter(index, view.id, config.unitViewsInModal)} key={view.id} aria-haspopup={config.unitViewsInModal ? "dialog" : undefined} disabled={Boolean(config.unitViewsInModal && viewIndex > unlockedViewForChapter(index))}><b>{view.icon}</b><span>{view.label}</span></button>)}</div> : null}
              </div>;
            })}</div>
            <footer><span>Halim&apos;s Life Learning Project</span><b>i</b></footer>
          </article>
        </div>}
