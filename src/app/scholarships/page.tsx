...
export default async function ScholarshipsPage({searchParams}:{searchParams:Promise<{country?:string;q?:string;filter?:string}>}){
  ...
  const activeCountry=params.country==="japan"?"japan":"usa";
  const query=(params.q??"").trim();
  const activeFilter=params.filter??"";
  const countryGuides=activeCountry==="japan"?japanGuides:usaGuides;
  const queryText=query.toLocaleLowerCase();
  const filteredGuides=countryGuides.filter((guide)=>{
    const searchable=[guide.university,guide.title,guide.summary,guide.label,guide.funding,guide.audience,...guide.highlights,...guide.fit].join(" ").toLocaleLowerCase();
  ...
  const filterOptions=[{value:"funded",label:"Fully funded"},{value:"active",label:"Active lead"},{value:"email",label:"Email first"},{value:"deadline",label:"Deadline"},{value:"phd",label:"PhD"},{value:"masters",label:"Master’s"}];
  ...
  <section className={styles.filters}><form action="/scholarships#guides" method="get"><Icon name="search"/><input name="q" defaultValue={query} placeholder="University, professor বা research area খুঁজুন"/><input type="hidden" name="country" value={activeCountry}/>{activeFilter&&<input type="hidden" name="filter" value={activeFilter}/>}</form><Link className={styles.allFilter} aria-current={!query&&!activeFilter?"page":undefined} href={makeHref({q:"",filter:""})}>সব</Link><Link aria-current={activeCountry==="usa"?"page":undefined} href={makeHref({country:"usa"})}>🇺🇸 USA · {usaGuides.length} guides</Link><Link aria-current={activeCountry==="japan"?"page":undefined} href={makeHref({country:"japan"})}>🇯🇵 Japan · {japanGuides.length} guides</Link><div className={styles.chips}>{filterOptions.map(option=><Link aria-current={activeFilter===option.value?"page":undefined} href={makeHref({filter:activeFilter===option.value?"":option.value})} key={option.value}>{option.label}</Link>)}</div></section>
