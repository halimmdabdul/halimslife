...
export const metadata: Metadata = { title:"Scholarships | USA, Canada & Japan Graduate Funding Guides", description:"Practical, source-checked graduate funding and scholarship guides for the USA, Canada and Japan.", alternates:{canonical:"/scholarships"} };
...
export default async function ScholarshipsPage({searchParams}:{searchParams:Promise<{country?:string;q?:string;filter?:string}>}){
  ...
  const activeCountry=params.country==="japan"?"japan":params.country==="canada"?"canada":"usa";
  const query=(params.q??"").trim();
  const activeFilter=params.filter??"";
  const countryGuides=activeCountry==="japan"?japanGuides:activeCountry==="canada"?canadaGuides:usaGuides;
