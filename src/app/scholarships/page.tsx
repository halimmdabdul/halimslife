...
export const metadata: Metadata = { title:"Scholarships | USA, Canada, South Korea & Japan", description:"Practical, source-checked graduate funding and scholarship guides for the USA, Canada, South Korea and Japan.", alternates:{canonical:"/scholarships"} };
...
export default async function ScholarshipsPage({searchParams}:{searchParams:Promise<{country?:string;q?:string;filter?:string}>}){
  ...
  const activeCountry=params.country==="japan"?"japan":params.country==="canada"?"canada":params.country==="korea"?"korea":"usa";
  const query=(params.q??"").trim();
  const activeFilter=params.filter??"";
  const countryGuides=activeCountry==="japan"?japanGuides:activeCountry==="canada"?canadaGuides:activeCountry==="korea"?koreaGuides:usaGuides;
