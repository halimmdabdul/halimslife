...
export default async function ScholarshipsPage({searchParams}:{searchParams:Promise<{country?:string;q?:string;filter?:string}>}){
  ...
  const filteredGuides=countryGuides.filter((guide)=>{
    const searchable=[guide.university,guide.title,guide.englishBusinessPrograms??"",guide.summary,guide.label,guide.funding,guide.audience,...guide.highlights,...guide.fit].join(" ").toLocaleLowerCase();
    ...
    return true;
  }).sort((a,b)=>activeFilter==="business"?(a.businessPriority??Number.MAX_SAFE_INTEGER)-(b.businessPriority??Number.MAX_SAFE_INTEGER):0);
  ...
  const filterOptions=[{value:"business",label:"English Business/MBA"},{value:"funded",label:"Fully funded"},{value:"active",label:"Active lead"},{value:"email",label:"Email first"},{value:"deadline",label:"Deadline"},{value:"phd",label:"PhD"},{value:"masters",label:"Master’s"}];
