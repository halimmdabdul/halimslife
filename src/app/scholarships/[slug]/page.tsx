...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const guide=getScholarshipGuide(slug);if(!guide)return{};const ogImage=(guide.country==="japan"?japanPanorama:alabamaCampus).src;return{title:guide.title,description:guide.summary,alternates:{canonical:`/scholarships/${guide.slug}`},openGraph:{title:guide.title,description:guide.summary,url:`/scholarships/${guide.slug}`,type:"article",images:[ogImage]}}}
...
export default async function ScholarshipDetailPage({params}:{params:Promise<{slug:string}>}){
  ...
  const countryLabel=guide.country==="japan"?"Japan":guide.country==="canada"?"Canada":guide.country==="korea"?"South Korea":guide.country==="switzerland"?"Switzerland":guide.country==="italy"?"Italy":"USA";
  const ctaImage=guide.country==="japan"?japanPanorama:alabamaCampus;
  const structured={"@context":"https://schema.org","@type":"Article",headline:guide.title,description:guide.summary,dateModified:guide.reviewedOn??undefined,mainEntityOfPage:`https://halimslife.com/scholarships/${guide.slug}`,author:{"@type":"Person",name:"Halim Md Abdul",url:"https://halimslife.com/about"}};
  const breadcrumb={"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[
    {"@type":"ListItem",position:1,name:"Home",item:"https://halimslife.com/"},
    {"@type":"ListItem",position:2,name:"Scholarships",item:"https://halimslife.com/scholarships"},
    {"@type":"ListItem",position:3,name:countryLabel,item:`https://halimslife.com/scholarships?country=${guide.country}`},
    {"@type":"ListItem",position:4,name:guide.university,item:`https://halimslife.com/scholarships/${guide.slug}`},
  ]};
  return <InnerPageShell><article className={styles.page}>
    <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structured).replace(/</g,"<")}}/>
    <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(breadcrumb).replace(/</g,"<")}}/>
    <header className={styles.hero}><div className={styles.container}><nav><Link href="/scholarships">Scholarships</Link><span>/</span><Link href={`/scholarships?country=${guide.country}`}>{countryLabel}</Link><span>/</span><span>{guide.university}</span></nav><div className={styles.heroGrid}><div><span className={styles.label}>{guide.label}</span><p className={styles.university}>{guide.university}</p><h1>{guide.title}</h1><p>{guide.summary}</p><div className={styles.heroActions}><a href={guide.officialLinks[0]?.href??"#official-sources"} target="_blank" rel="noreferrer">Official admission page ↗</a><a href="#application-steps">Application steps ↓</a></div></div><div className={styles.heroArt}><Image src={navigatorHero} alt="Graduate funding and application research illustration" fill priority sizes="48vw"/></div></div><div className={styles.quickFacts}><article><Icon name="funding"/><div><b>Funding</b><p>{guide.funding}</p></div></article><article><Icon name="document"/><div><b>Includes</b><p>Tuition · fees · stipend</p></div></article><article><Icon name="calendar"/><div><b>Typical length</b><p>{guide.duration}</p></div></article><article><Icon name="calendar"/><div><b>Deadline</b><p><DeadlineBadge deadline={guide.applicationDeadline}/></p></div></article><article><Icon name="people"/><div><b>Applicants</b><p>{guide.audience}</p></div></article></div><div className={styles.eligibility}><Icon name="check"/>Eligibility, degree requirement ও profile-fit official guide থেকে verify করুন।</div></div></header>
